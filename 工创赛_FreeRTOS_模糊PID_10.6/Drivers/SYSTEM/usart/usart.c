...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
    ...
    if(huart->Instance == USART_UX)                             /* 如果是串口1，进行串口1 MSP初始化 */
    {
        ...
        HAL_NVIC_SetPriority(USART_UX_IRQn, 3, 3);              /* 抢占优先级3，子优先级3 */
		    ...
		    if(huart->Instance == USART2_UX)                             /* 如果是串口2，进行串口2 MSP初始化 */
    {
...
uint8_t ucTemp;
uint16_t tt=0;//进入串口中断自增
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    if(huart->Instance == USART_UX)             /* 如果是串口1 */
    {
				//HAL_UART_Receive_IT(&g_uart1_handle, (uint8_t *)&ucTemp,1);	
...
{ 
    ...
    while (HAL_UART_Receive_IT(&g_uart1_handle, (uint8_t *)&ucTemp,1)	 != HAL_OK)
    {
        timeout++;                              /* 超时处理 */
        if (timeout > maxDelay)
        {
...
uint8_t g_rx2_buffer[RX2BUFFERSIZE];                  /* HAL库使用的串口接收缓冲 */
...
void usart2_init(uint32_t baudrate)
{
    ...
    /* 该函数会开启接收中断：标志位UART_IT_RXNE，并且设置接收缓冲以及接收缓冲接收最大数据量 */
    HAL_UART_Receive_IT(&g_uart2_handle, (uint8_t *)g_rx2_buffer, RX2BUFFERSIZE);
}
